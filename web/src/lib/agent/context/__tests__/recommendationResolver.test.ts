...
test("canonical recommendation outranks session, chart and history", () => {
  const selected = resolveActiveRecommendationContext({ candidates: [rec("h", "history"), rec("c", "chart"), rec("s", "session"), rec("p", "canonical")] });
  assert.equal(selected?.id, "p");
});

test("newer canonical recommendation for the same symbol is selected", () => {
  const selected = resolveActiveRecommendationContext({ candidates: [
...
test("terminal and expired historical recommendations are not active", () => {
  const selected = resolveActiveRecommendationContext({ candidates: [rec("p", "canonical", { status: "expired" }), rec("h", "history", { status: "cancelled" })] });
...
test("matches symbol and timeframe and supports chart restore", () => {
  const selected = resolveActiveRecommendationContext({
    ...
    candidates: [rec("other-symbol", "canonical", { symbol: "EURUSD" }), rec("other-tf", "session", { timeframe: "1h" }), rec("chart", "chart")],
  });
