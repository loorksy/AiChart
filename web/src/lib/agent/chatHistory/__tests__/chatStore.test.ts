...
test("agent chat history store persists, loads, titles, and scopes chats", async () => {
  ...
  const list = await store.listChats(owner);
  ...
  assert.equal((await store.getMessages(attacker, chat.id)).length, 0);
  assert.equal(
  ...
  assert.equal(
    (await store.getMessages(owner, chat.id)).length,
    2,
    "owner's messages are untouched by the attacker",
  );

  // --- New chat is a distinct, fresh session ---
  const chat2 = await store.createChat({ userId: owner });
  assert.notEqual(chat2.id, chat.id);
  const list2 = await store.listChats(owner);
