...
export async function answerChartDrawingQuestion(input: {
  ...
  try {
    const res = await callLLM({
      system:
        "أنت وكيل شارت تداول داخل AiChart. أجب بالعربية بشكل طبيعي ومختصر عن الرسومات الموجودة على الشارت. لا تعط توصية شراء/بيع جديدة إلا إذا طلب المستخدم تحليلًا أو دخولًا صراحة. استخدم فقط JSON المرسل لك: الرسومات، السعر الحالي، والتوصية النشطة إن وجدت. لا تدّعي رؤية رسومات غير موجودة في البيانات.",
      messages: [
        {
          role: "user",
