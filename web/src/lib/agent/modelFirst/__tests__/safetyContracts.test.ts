...
function validPlan(): ModelTradePlan {
  return {
    ...
    currentPriceContext: "Quote is inside the entry zone.",
    timeframeAlignment: [],
    entryZone: { low: 1.1, high: 1.101, preferred: 1.1005 },
    invalidation: 1.0985,
    stopLoss: 1.099,
    targets: [
      { price: 1.102, rationale: "First objective" },
      { price: 1.103, rationale: "Second objective" },
    ],
    ...
    numericTimeframesUsed: ["5m", "15m", "1h", "4h"],
...
describe("complete technical validator", () => {
  ...
  it("does not rewrite trade activation and reports missing technical fields", () => {
    const plan = {
      ...
      invalidation: null,
      targets: [{ price: 1.102, rationale: "Only one" }],
    };
    const result = validateTradePlanTechnically({
      plan,
      currentPrice: null,
