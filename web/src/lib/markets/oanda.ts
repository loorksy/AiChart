/**
 * OANDA v20 client — an ANALYSIS-ONLY market data source (plan "إعادة OANDA
 * كمصدر بيانات تحليلي فقط"). This module is a bare HTTP client and nothing
 * more: it is NEVER imported by `resolveMarketDataSource`/`fetchOhlc` (the
 * chart-facing, single-source-of-truth pipe) or by anything in Lonora's live
 * decision path (`agent/orchestrator.ts`, `recommendations/
 * recommendationTracker.ts`, `agent/marketContext/*`). Its only legitimate
 * callers are `web/src/lib/candles/oandaBackfillService.ts` (writes into
 * `market_candles` under `source='oanda'` via `analysisCandleRepository.ts`)
 * and Quant Agent's cron-context candle fetch. The user's own linked
 * MetaTrader account remains the ONLY data ever rendered on the visual chart
 * or used for live trade math — see `analysisCandleRepository.ts`'s header
...
/** How far back history may paginate (10 years). */
...
/** True when an OANDA token is configured (analysis source active). */
...
/**
 * Historical + still-forming candles for a forex/metal instrument from OANDA.
 */
export async function fetchOandaCandles(
  ...
  if (opts?.fromMs != null && opts?.toMs != null && opts.toMs > opts.fromMs) {
    // OANDA returns HTTP 400 when `to` is in the future or a range spans
    // >5000 candles. Clamp both `to` (to now) and `from`.
  ...
  for (const r of rows) {
    // Keep the still-forming last bar — analysis callers may want it too.
    if (!r.mid) continue;
  ...
  if (opts?.fromMs != null && opts?.toMs != null) {
    ...
    hasMore = out.length > 0 && oldest > Date.now() - HISTORY_LOOKBACK_MS + barDurationMs(interval);
  } else if (opts?.beforeMs && opts.beforeMs > 0) {
    const oldest = out[0]?.time ?? 0;
    hasMore = out.length > 0 && oldest > Date.now() - HISTORY_LOOKBACK_MS + barDurationMs(interval);
  } else {
...
/**
 * Live bid/ask/mid for one or more symbols from OANDA. Requires
 * OANDA_ACCOUNT_ID. Analysis use only — never presented as a tradeable quote
 * for execution (see the plan's MCP tool table: `get_market_price` stays
 * MT-only for exactly this reason).
 */
export async function fetchOandaPricing(symbols: string[]): Promise<OandaQuote[]> {
  ...
  const instruments = symbols.map((s) => toOandaInstrument(s)).filter((i): i is string => i != null);
...
/**
 * OANDA's own tradable instrument universe (forex + metals + CFDs) — analysis
 * reference only, never presented as the user's own broker's tradable
 * universe (see the plan's MCP tool table: `list_instruments` stays MT-only).
 */
