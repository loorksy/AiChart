 ...
 *
 * That is how METAAPI_TOKEN, the three OANDA keys and GEMINI_API_KEY ended up
 * unreachable while three separate surfaces told the operator to "add it from
 * the platform panel". This test walks the real source tree so the next key
 * added to a getPlatformValue call has to come with a field.
 */
...
test("credentials are stored encrypted, plain values are not", () => {
  ...
  for (const key of ["METAAPI_TOKEN", "OANDA_API_TOKEN", "GEMINI_API_KEY"]) {
