...
| [`Text Document جديد (2).txt`](../Text%20Document%20جديد%20(2).txt) | **فارغ (0 بايت)** — المصدر كان فحص الكود + هذا المستند |
...
|---|----------|----------|-------|--------|------------------|
| 1 | تفعيل `monthly_loss_limit_pct` | عالية | صغير | **منفّذ** | `riskGuard.ts`, `store.ts` |
| 2 | توحيد بيئة Binance (testnet/prod) | عالية | صغير | **منفّذ** | `execution.ts`, `binance.ts` |
| 3 | خيار الموافقة في الإعدادات | عالية | صغير | **منفّذ** | `SettingsClient.tsx` |
| 4 | موافقة صفقات من الويب في advisory | عالية | صغير | **منفّذ** | `chat/route.ts`, `tradeFlow.ts` |
| 5 | موافقة/رفض داخل المحادثة | متوسطة | متوسط | **منفّذ** | `ChatSquareClient.tsx` |
| 6 | تنبيه النوايا المعلّقة على اللوحة | متوسطة | صغير | **منفّذ** | `DashboardClient.tsx` |
| 7 | تحديد نطاق المراقبة 24/7 | متوسطة | متوسط | **منفّذ** | `monitorRunner.ts` |
| 8 | جدولة Cron على VPS | متوسطة | تشغيلي | **جاهز للنشر** | `infra/aichart.cron`, `vps-verify-cron.mjs` |
| 9 | ربط `/market` بالتنقل | متوسطة | صغير | **منفّذ** | `AppShell.tsx` |
| 10 | إغلاق الصفقات + PnL | منخفضة | كبير | **منفّذ** | `tradeClose.ts`, `store.ts` |
| 11 | أوامر OCO لـ SL/TP | منخفضة | كبير | **منفّذ** (+ مزامنة) | `binance.ts`, `execution.ts`, `tradeClose.ts` |
| 12 | Kill Switch يغلق المراكز | منخفضة | كبير | **منفّذ** | `tradeClose.ts`, `riskGuard.ts` |
| 13 | حوار المبتدئ الكامل | منخفضة | متوسط | **منفّذ** | `OnboardingClient.tsx`, `/api/onboarding/suggest` |
| 14 | إشعارات داخل الموقع | منخفضة | كبير | **منفّذ** | `alert_log`, `NotificationPanel.tsx`, `AppHeader.tsx` |
...
الملف [`Text Document جديد (2).txt`](../Text%20Document%20جديد%20(2).txt) كان **فارغاً (0 بايت)** أيضاً — الخطة المُنفَّذة بُنيت على فحص الكود مقابل هذا المستند.
...
|---------|--------|----------|---------------------------|--------|
| 2026-06-10 | فحص كود | `(2).txt` فارغ | مرفوض كمصدر | الاعتماد على `SUGGESTIONS_FEASIBLE.md` + الكود |
| — | — | *(لا اقتراحات خارجية أخرى)* | — | — |
