...
| 15 | §13 H.1 | ملف تكلفة جلسي **حي** من عروض الوسيط | `partial` | `SESSION_SPREAD_MULTIPLIER` مضاعِفات ثابتة، لا تجميع مرصود؛ مصدر العروض الحية (EA) أُزيل بلا بديل |
...
| 15 — ملف التكلفة الحي | كانت العيّنات تُجمع من عروض EA عبر كرون المراقبة → `cost_samples`؛ مع إزالة جسر EA أُزيل مصدر التجميع (`sampleLiveCosts`) بلا بديل — `cost_samples` لن تُملأ بعد الآن. القراءة والتجميع (median/p90/انزلاق-التشتت لكل رمز×جلسة، fallback ثابت **موسوم** `static_model`، التقادم جزء من الجواب) ما زالت تعمل على ما تراكم سابقاً | `regressed` | `liveCostProfile.ts`؛ `liveCostProfile.test.ts` (7) |
