...
| 7 | Quote-staleness / spread ceiling checked only on some routes; an approval-flow or auto order can fill across a blown-out spread on a stale heartbeat | CONFIRMED | High | **FIXED** — the EA preflight (`checkForexTradePreflight`) now runs at the choke point for `mt_ea` intents before sizing. | `executionMatrix.test.ts` "a stale/absent live quote → refused by preflight" |
...
| 16 | `market.spread` always null → the decision engine never sees real cost | CONFIRMED | High | **PART-FIXED** — `executionCost` was gated on the observed spread and dropped the real live-EA cost profile computed upstream; it is now built whenever a live cost OR an observed spread exists, so live cost reaches the model. Part 2 (wiring the raw observed spread through the 4 entry points, which also re-arms the spread-drift trigger #17) remains OPEN. | — |
