...
echo
# Prove catalog revision is live in source tree
grep -n "CATALOG_SPEC_REVISION" "$REAL/web/src/lib/strategies/catalog.ts" || true
docker exec aichart-production-research-1 grep -n "discard" /app/app/backtest/engine.py | head -3 || true
