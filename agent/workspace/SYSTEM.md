...
- BUY/SELL must bind to a real candidate with real entry, stop, and target levels.
...
<!-- mcp-core-start -->
Session start: get_agent_capabilities → get_account_overview → wait. Per analysis: fetch fresh market evidence → choose BUY/SELL/WAIT → bind BUY/SELL to a real candidate. Per execution: get_trade_readiness → explicit approval → open_trade. Never pass lots, notional, leverage, or balance overrides.
<!-- mcp-core-end -->
