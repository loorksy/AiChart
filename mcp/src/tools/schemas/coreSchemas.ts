...
export const CORE_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    domain: "core",
    description:
      "Captures an image of the live TradingView chart for one symbol and interval — from the operator's own tab when one is open (including a background tab), else from the platform's hosted chart session (an isolated headless tab running the same internal chart page), so captures work with no operator tab at all. Either way the pixels are TradingView's client screenshot (image_source=tradingview_capture, drawings/studies as actually rendered, the two-shot pair: ~400-candle context + ~90-candle zoom). Requests for the same symbol/interval inside the same short moment (~15s) share one capture — never an older one: a snapshot past that window is re-captured, and NO stored image is ever served as fresh. When NEITHER tab can answer, NO image is returned at all — the call fails honestly (HTTP 503 with a reason: no_live_session, layout_not_found, capture_timeout, upload_failed, missing_shots, host_unreachable, or host_not_ready) and you must tell the operator no chart was captured rather than describe or imply one; when drawings_included comes back false, visual_confirmation must be not_checked. The PNG is attached inline AND returned as display_markdown — ALWAYS paste display_markdown verbatim (the link expires after ~3 minutes). When: with every recommendation, or whenever the operator should see the current chart. Not for multiple timeframes at once — capture_multi_timeframe_snapshot does that in one parallel call. Default inline_image=true, include_drawings=true, include_studies=true. read-only on market; side-effect: capture.",
    inputSchema: {
      symbol: zSymbol,
      interval: zInterval,
      market: zMarket,
      ...
      include_drawings: zLooseBoolean
        .optional()
        .describe(
          "Include drawings already rendered on the live TradingView chart (default true). When false the same live TradingView path captures a clean, undrawn chart.",
        ),
