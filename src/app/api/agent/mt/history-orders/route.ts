...
/**
 * Bridge: completed MT5 orders, straight from MetaApi — by ticket, by
...
export const GET = withBridge(async ({ req, userId }) => {
  ...
  if (mode === "range") {
    ...
    return conn.getHistoryOrdersByTimeRange(from, to, offset, limit);
  }
  throw new ApiError(400, "mode مطلوب: ticket أو position أو range.");
}, { routeKey: "/api/agent/mt/history-orders" });
