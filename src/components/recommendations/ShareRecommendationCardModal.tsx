 ...
 * recommendation card immediately. PNG capture runs in the background for
 * download / Web Share only — the visible card is never swapped for a snapshot.
 */
"use client";
...
import { canShareFiles } from "@/lib/recommendations/profitCard";
import {
...
import {
  captureRecommendationCardPng,
  REC_CARD_CAPTURE_MIN_HEIGHT,
  REC_CARD_CAPTURE_WIDTH,
} from "@/lib/recommendations/recommendationCardCapture";
import { isUsablePngBlob, loadProfitCardLogoDataUrl } from "@/lib/recommendations/profitCardCapture";
...
export function ShareRecommendationCardModal({
  open,
  onClose,
  rec,
  livePrice,
  ...
  const model = useMemo(
    () => buildRecommendationCardModel(rec, { locale, livePrice, now: Date.now() }),
    [rec, locale, livePrice],
  );
  const labels = useMemo(() => recommendationCardLabels(model), [model]);
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<Blob | null> => {
      ...
      try {
        const blob = await captureRecommendationCardPng({
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Popup
          ref={sheetRef}
          dir={dir}
          data-testid="recommendation-card-share-modal"
          className={cn(
        ...
        >
          <div
            {...handleProps}
            data-testid="recommendation-card-share-handle"
          ...
          >
            <Dialog.Title className="min-w-0 flex-1 truncate text-[15px] font-semibold tracking-tight">
              {t("rec_card.share_title")}
            </Dialog.Title>
            <Dialog.Close className="sr-only md:hidden" aria-label={t("agent.signal.close_report")}>
            ...
            <Dialog.Close
              ...
              data-testid="recommendation-card-share-close"
            >
          ...
          >
            <div className="relative mx-auto w-full max-w-[360px]">
              <div ref={liveRef} data-testid="recommendation-card-live-preview">
                <RecommendationCard model={model} labels={labels} logoSrc={logoSrc ?? undefined} />
              </div>
            </div>
            {failed ? (
              <p className="mt-3 text-center text-[12px] text-muted-foreground">{t("rec_card.failed")}</p>
            ) : null}
          </div>
          ...
          <div className="flex shrink-0 flex-wrap gap-2 border-t border-border/50 px-4 py-3">
            <button
              ...
              data-testid="recommendation-card-download"
            ...
            <button
              ...
              data-testid="recommendation-card-share-action"
            ...
            >
              ...
              {copied ? t("rec_card.link_copied") : t("rec_card.share")}
            </button>
          </div>
          ...
          {open && typeof document !== "undefined"
            ? createPortal(
                <div
                  aria-hidden
                  data-testid="recommendation-card-capture"
                  ref={captureRef}
                  ...
                  style={{
                    ...
                    height: REC_CARD_CAPTURE_MIN_HEIGHT,
