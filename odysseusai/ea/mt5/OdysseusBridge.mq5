//+------------------------------------------------------------------+
//|                                              OdysseusBridge.mq5    |
//|       Connects a MetaTrader 5 terminal to the Odysseus platform     |
//|       via the self-hosted EA bridge API (heartbeat + commands).  |
//+------------------------------------------------------------------+
#property copyright "Odysseus"
#property link      "https://your-odysseus-host"
...
//--- Inputs --------------------------------------------------------
input string  ApiBase          = "https://your-odysseus-host"; // Odysseus base URL (ApiKey = EaToken)
input string  EaToken          = "";                            // EA token from Odysseus trading settings
...
int OnInit()
{
   ...
   {
      Print("OdysseusBridge: EaToken is not set. Open EA properties and paste your token.");
      return(INIT_FAILED);
   }
   EventSetTimer(1);
   Print("OdysseusBridge MT5 v", EA_VERSION, " started. Base=", ApiBase, " hb=", HeartbeatSeconds, "s poll=", PollIntervalMs, "ms quotes=", QuoteFlushSeconds, "s chartQuoteMs=", ChartQuoteThrottleMs);
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED))
      Print("OdysseusBridge: AutoTrading button is OFF. Enable it once, and UNTICK ",
...
void SendHeartbeat()
{
   ...
   string resp = "";
   if(!HttpPost("/api/ea-bridge/heartbeat", body, resp))
   {
      g_hb_failures++;
      g_connected = false;
      if(g_hb_failures == 1 || g_hb_failures % 10 == 0)
         Print("OdysseusBridge: heartbeat failed (", g_hb_failures, "). Check WebRequest URL and network.");
      UpdateStatusPanel();
      return;
   }
   if(g_hb_failures > 0)
   {
      Print("OdysseusBridge: heartbeat restored after ", g_hb_failures, " failure(s).");
      g_hb_failures = 0;
   }
   g_connected = true;
...
void UpdateStatusPanel()
{
   ...
   {
      g_algo_warned = true;
      Print("OdysseusBridge: AutoTrading is OFF — enable the green AutoTrading button. ",
   ...
   string s = "";
   s += "Odysseus Bridge  v" + EA_VERSION + "\n";
   s += "--------------------------------\n";
...
{
   string resp = "";
   if(!HttpGet("/api/ea-bridge/commands", resp))
      return;
                          ...
                          uint &retcode, long &outTicket, double &outPrice)
{
   ...
   if(!OrderCheck(req, chk))
   {
      ...
      Print("OdysseusBridge: ", g_last_trade_error);
      return false;
   }
                           ...
                           uint deviation, uint &retcode, long &outTicket, double &outPrice)
{
   ...
   Print("OdysseusBridge: retry market deal without SL/TP then modify");
...
{
   string msg = "OdysseusBridge: ORDER " + context +
                " sym=" + sym + " side=" + side +
...
bool TryMarketOrder(string sym, string side, double lots, double sl, double tp, uint &retcode)
{
   ...
   for(int attempt = 0; attempt < MathMax(1, MaxRetries); attempt++)
   {
      ...
      bool ok = false;
      if(side == "buy")
         ok = trade.Buy(lots, sym, price, sl, tp, "Odysseus");
      else
         ok = trade.Sell(lots, sym, price, sl, tp, "Odysseus");
                     ...
                     double sl, double tp, uint &retcode, long &placedTicket)
{
   ...
   for(int attempt = 0; attempt < MathMax(1, MaxRetries); attempt++)
   {
      ...
      {
         if(isBuy)
            ok = trade.BuyLimit(lots, price, sym, sl, tp, ORDER_TIME_GTC, 0, "Odysseus");
         else
            ok = trade.SellLimit(lots, price, sym, sl, tp, ORDER_TIME_GTC, 0, "Odysseus");
      }
      else if(orderType == "stop")
      {
         if(isBuy)
            ok = trade.BuyStop(lots, price, sym, sl, tp, ORDER_TIME_GTC, 0, "Odysseus");
         else
            ok = trade.SellStop(lots, price, sym, sl, tp, ORDER_TIME_GTC, 0, "Odysseus");
      }
...
void FlushChartSymbolQuote()
{
   ...
   string resp = "";
   if(!HttpPost("/api/ea-bridge/quotes", body, resp))
   {
      g_quote_failures++;
      if(g_quote_failures == 1 || g_quote_failures % 10 == 0)
         Print("OdysseusBridge: chart quote push failed (", g_quote_failures, ").");
      return;
   }
   if(g_quote_failures > 0)
   {
      Print("OdysseusBridge: chart quote push restored after ", g_quote_failures, " failure(s).");
      g_quote_failures = 0;
   }
...
void FlushLiveQuotes()
{
   ...
   if(!HttpPost("/api/ea-bridge/quotes", body, resp))
   {
      g_quote_failures++;
      if(g_quote_failures == 1 || g_quote_failures % 10 == 0)
         Print("OdysseusBridge: batch quote push failed (", g_quote_failures, ").");
      return;
   }
   if(g_quote_failures > 0)
   {
      Print("OdysseusBridge: batch quote push restored after ", g_quote_failures, " failure(s).");
      g_quote_failures = 0;
   }
}
...
void SendTradeEvent(string eventType, string sym, long dealId)
{
   ...
   body += "}";
   string resp = "";
   HttpPost("/api/ea-bridge/event", body, resp);
}
...
void ProcessHeartbeatFlags(string resp)
{
   ...
   if(killOn && !g_trading_halted)
   {
      ...
      Print("OdysseusBridge: KILL SWITCH active — new trades blocked.");
   }
   else if(!killOn && g_trading_halted)
   {
      g_trading_halted = false;
      Print("OdysseusBridge: Kill switch cleared — trading resumed.");
   }

   if(closeOpen)
   {
      Print("OdysseusBridge: Kill switch requested close of all open positions.");
      CloseAllPositions();
   }
...
{
   Print("OdysseusBridge: server reconnect",
         fullReconnect ? " (full)" : "",
...
void AckCommand(long id, string status, long ticket, double price, double lots, string err)
{
   ...
   body += "}}";
   string resp = "";
   HttpPost("/api/ea-bridge/commands/" + (string)id + "/ack", body, resp);
}
...
datetime TimeFromBarOffset(string sym, ENUM_TIMEFRAMES tf, int barOffset, double unixOverride)
{
   ...
   Print("OdysseusBridge: iTime empty for ", sym, " tf=", (int)tf, " offset=", barOffset);
   return TimeCurrent();
}
                   ...
                   datetime &times[], double &prices[], int n)
{
   ...
   {
      Print("OdysseusBridge: unsupported drawing type: ", dtype);
      return;
   }
...
{
   string url = ApiBase + path;
   string boundary = "----Odysseus" + (string)GetTickCount();
...
void DrawAndCapture(long id, string payload)
{
   ...
   string uploadResp = "";
   if(!HttpPostMultipart("/api/ea-bridge/chart-upload", recId, captureKey, shotName, fileData, uploadResp))
   {
